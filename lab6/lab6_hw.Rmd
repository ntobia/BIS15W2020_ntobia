...
##Remember, it's always nice to make a data table to go along with plots
```{r}
...
global_life_exp
```
##A bar plot works, but the relationship might be shown more clearly using a line graph.

**3. How do the distributions of life expectancy compare for the years 1952 and 2007? _Challenge: Can you put both distributions on a single plot?_**
##Another option would be to using a density plot - geom_density()
```{r}
gapminder %>% 
  group_by(year) %>% 
  filter(year == "1952" | year == "2007") %>% 
...
##Make sure your code generates the summary you create
```{r}
...
##Love the attention to detail on the axes.
```{r}
lifeExp_region %>% 
...
##I like giving the continents seperate colors
```{r}
gapminder %>% 
...
##you can also use geom_smooth() to add a regression line and make the relationship that much more obvious.
##Note you've ased R to color by continent on a plot of countries, this is why you're getting a warning in the markdown.
```{r}
gapminder %>% 
  select(country, continent, year, lifeExp, gdpPercap,) %>%
