...
##Let R evaluate the code so we can see the results
```{r, echo=T}
...
#your code works fine, but here's another way to do this that's a little shorter
#blackjack_earnings <- sum(blackjack_earnings, na.rm + TRUE)
```
...
```{r, echo=T}
total_week <- blackjack_earnings+roulette_earnings
#again, this works, but we can make it a s little cleaner
```
