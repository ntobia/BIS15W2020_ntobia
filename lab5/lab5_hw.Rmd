...
##Is this change necessary for the amphibio dataset?
```{r}
...
##How does this affect your interpretation of NAs? - Note that in this dataset NAs could refer to both non-applicable data as wel as missing data.
```{r}
amniota_NA %>% 
  group_by(class) %>% 
...
##Remember to include a chart (data table) with your graph
```{r}
amniota_NA %>% 
  group_by(class) %>% 
  ggplot(aes(x = class))+
